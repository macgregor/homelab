...
- **MikroTik syslog** -- The router forwards log topics via UDP syslog directly to VictoriaLogs' built-in syslog receiver (exposed via a MetalLB LoadBalancer). Firewall drop and accept rules use rate-limited `action=log` rules placed before each target rule to sample events without flooding. Logging topics and firewall log rules are configured in `ansible/mikrotik-configure.yml`.
